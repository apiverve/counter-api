...
class CounterValidationException implements Exception {
  final List<String> errors;

  CounterValidationException(this.errors);

  @override
  String toString() => 'CounterValidationException: ${errors.join("; ")}';
}
...
/// Cloud Counter API client.
///
/// For more information, visit: https://apiverve.com/marketplace/counter?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [id] (required) - The ID of the counter (e.g., test_counter)
class CounterClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'id': ValidationRule(type: 'string', required: true),
  };

  CounterClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/counter',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CounterValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CounterValidationException(errors);
    }
  }

  /// Execute a request to the Cloud Counter API.
  ///
  /// Parameters are validated before sending the request.
  Future<CounterResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CounterException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CounterResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CounterException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CounterException('Resource not found');
      } else {
        throw CounterException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CounterException) rethrow;
      throw CounterException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Cloud Counter API client.
class CounterException implements Exception {
  ...
  @override
  String toString() => 'CounterException: $message';
}
